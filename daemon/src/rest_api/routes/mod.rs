...
mod test {
    ...
    /// Verifies a GET /products responds with an OK response with a
    ...
    fn test_list_products() {
        ...
        let request = srv.client(http::Method::GET, "/products").finish().unwrap();
        let response = srv.execute(request.send()).unwrap();
        assert!(response.status().is_success());
    ...
    ///
    /// Verifies a GET /products/{id} responds with an OK response
    ...
    fn test_fetch_product_ok() {
        ...
        let request = srv
            .client(
                http::Method::GET,
                &format!("/products/{}", "041205707820".to_string()),
            )
            .finish()
            .unwrap();
        let response = srv.execute(request.send()).unwrap();
    ...
    ///
    /// Verifies a GET /products/{id} responds with a Not Found error
    ...
    fn test_fetch_product_not_found() {
        ...
        clear_product_table(&test_pool.get().unwrap());
        let request = srv
            .client(http::Method::GET, "/products/not_in_database")
            .finish()
            .unwrap();
        let response = srv.execute(request.send()).unwrap();
        assert_eq!(response.status(), http::StatusCode::NOT_FOUND);
